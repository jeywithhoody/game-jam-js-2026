...
export class Level1 extends Level {
  ...
  private dryerSprites: Map<string, Phaser.GameObjects.Sprite> = new Map();
  private selectedWasherPosition: { x: number; y: number } | null = null;
  private selectedDryerPosition: { x: number; y: number } | null = null;
  ...
  create() {
    ...
    this.levelInfoScene = this.scene.get(
      SceneNames.LevelInfo,
    ) as LevelInfoScene;
    ...
    this.dryerSprites.forEach((sprite) => {
      this.levelZoneScene!.getContainer().add(sprite); // Add washer to container
    });
  ...
  private createDryerSprite(x: number, y: number): void {
    if (!this.levelGrid) return;

    // Get the world position for the dryer at grid (2, 0)
    const worldPos = this.levelGrid.getWorldPosition(x, y);

    // Create washer sprite at absolute position (will be converted to relative when added to container)
    const dryerSprite = this.add.sprite(
      this.levelZoneX + worldPos.x,
      this.levelZoneY + worldPos.y,
      "washer-machine-run1",
    );
    this.dryerSprites.set(`${x},${y}`, dryerSprite);
    console.log(this.dryerSprites);
    dryerSprite.setScale(0.3);
    dryerSprite.setCrop(118, 49, 342, 314);
    dryerSprite.setOrigin(0.75, 0.55);
    dryerSprite.setDepth(90); // Below robot (100) but above grid visuals

    // Create animation for the washer machine
    this.anims.create({
      key: `dryer-running-${x}-${y}`, // Unique key for each washer
      frames: [
        { key: "washer-machine-run1" },
        { key: "washer-machine-run2" },
        { key: "washer-machine-run3" },
  ...
  private createSortingStationSprite(x: number, y: number): void {
    ...
    clothSorting.setOrigin(0.33, 0.3);
  ...
  private onCardPlayed(data: {
    ...
    const success = this.playCard(
      data.cardType,
      data.cardSpeed as unknown as CardSpeed,
    );
  ...
  public playCard(cardType: CardType, cardSpeed: CardSpeed): boolean {
    ...
    if (success && cardType === CardType.Drop) {
      ...
      const actionZone = this.actionZoneSystem.canPerformAction(
        robotPos.x,
        robotPos.y,
        "put",
      );
      ...
      if (actionZone.can && actionZone.zoneId) {
        ...
        if (result.success) {
          ...
          if (dropIdx !== undefined)
            this.levelInfoScene?.markActionComplete(dropIdx);
    ...
    if (success && cardType === CardType.Take) {
      ...
      if (actionZone.can && actionZone.zoneId) {
        ...
        if (result.success) {
          ...
          if (takeIdx !== undefined)
            this.levelInfoScene?.markActionComplete(takeIdx);
        }
      }
    }
  ...
  protected onRobotMovementComplete(): void {
    ...
    if (actionZone.can && actionZone.zoneId) {
      const result = this.actionZoneSystem.performAction(actionZone.zoneId);
      console.log(
        `Robot moved to (${robotPos.x}, ${robotPos.y}) - checking action zones`,
        actionZone,
        result,
      );
      if (result.success) {
        ...
        if (moveIdx !== undefined)
          this.levelInfoScene?.markActionComplete(moveIdx);
        ...
        if (actionZone.zoneId === "washer-put" && actionZone.action === "put") {
          ...
          console.log(
            `Selected washer position: (${this.selectedWasherPosition.x}, ${this.selectedWasherPosition.y})`,
          );
          console.log(`Washer sprites available: ${this.washerSprites}`);
          this.animateWasher();
        }
        if (actionZone.zoneId === "dryer-put" && actionZone.action === "put") {
          this.selectedDryerPosition = { x: robotPos.x, y: robotPos.y };
          console.log("Triggering dryer animation from movement completion");
          console.log(
            `Selected dryer position: (${this.selectedDryerPosition.x}, ${this.selectedDryerPosition.y})`,
          );
          console.log(`Dryer sprites available: ${this.dryerSprites}`);
          this.animateDryer();
        }
        if (
          actionZone.zoneId === "sorting-station" &&
          actionZone.action === "take"
        ) {
          console.log(
            "Triggering sorting station animation from movement completion",
          );
  ...
  private animateWasher(): void {
    ...
    const washer = this.washerSprites.get(
      `${this.selectedWasherPosition.x},${this.selectedWasherPosition.y}`,
    );
    if (!washer) return;

    // Play the washer running animation
    washer.play(
      `washer-running-${this.selectedWasherPosition.x}-${this.selectedWasherPosition.y}`,
    );
  ...
  /**
   * Animate the dryer machine sprite
   */
  private animateDryer(): void {
    if (!this.selectedDryerPosition) return;
    const dryer = this.dryerSprites.get(
      `${this.selectedDryerPosition.x},${this.selectedDryerPosition.y}`,
    );
    if (!dryer) return;

    //Play the dryer running animation
    dryer.play(
      `dryer-running-${this.selectedDryerPosition.x}-${this.selectedDryerPosition.y}`,
    );
    this.scene.get(SceneNames.SoundScene).playMachineSound();
  ...
  private checkWinCondition(): void {
    ...
    const atStartPosition =
      robotPos.x === this.startX && robotPos.y === this.startY;
  ...
  private levelWon(): void {
    ...
    const transitionText = this.add
      .text(
        ...
        `Moving to Level 2 in 5 seconds...`,
        {
          fontSize: "24px",
          color: "#ffff00",
          fontFamily: "Arial",
